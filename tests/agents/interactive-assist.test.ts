...
describe("interactive assist", () => {
  ...
  it("resumes bootstrap from a partial profile without re-asking existing debt intake", async () => {
    const originalProfile = {
      debts: [
        {
          id: "card-a",
    ...
    const backupPath = `${profilePath}.bak`;
    const prompts: string[] = [];
    const answers = ["18000", "2500", "12000", "3200", "450", "0"];
  ...
  it("runs statement ingestion before manual debt intake for a new profile", async () => {
    const dir = mkdtempSync(join(tmpdir(), "runway-interactive-statements-"));
    tempDirs.push(dir);
    const profilePath = resolve(dir, "runway-profile.json");
    const prompts: string[] = [];
    const answers = ["yes", "18000", "2500", "12000", "3200", "450", "0", "0"];
    ...
    const outcome = await runInteractiveAssist({
      ...
      isInteractive: true,
      statementPaths: ["card.pdf"],
      ingestStatements: async ({ profile, statementPaths, ask }) => {
        expect(profile).toEqual({
          debts: [],
        });
        expect(statementPaths).toEqual(["card.pdf"]);

        const answer = await ask('Runway found a reviewed statement debt for "Card A". Keep it? (yes/no)');
    ...
    expect(prompts).toEqual([
      'Runway found a reviewed statement debt for "Card A". Keep it? (yes/no)',
      "How much available cash can runway use right now?",
      "How much cash should stay reserved?",
      "How much severance cash is available?",
      "What are the essential monthly obligations?",
  ...
  it("allows additional manual debts after statement-seeded intake", async () => {
    const dir = mkdtempSync(join(tmpdir(), "runway-interactive-statements-"));
    tempDirs.push(dir);
    const profilePath = resolve(dir, "runway-profile.json");
    const prompts: string[] = [];
    const answers = ["yes", "18000", "2500", "12000", "3200", "450", "1", "Medical Card", "800", "0.1899", "35", "0"];
    ...
    const outcome = await runInteractiveAssist({
      ...
      isInteractive: true,
      statementPaths: ["card.pdf"],
      ingestStatements: async ({ profile, ask }) => {
        const answer = await ask('Runway found a reviewed statement debt for "Card A". Keep it? (yes/no)');
        expect(answer).toBe("yes");
